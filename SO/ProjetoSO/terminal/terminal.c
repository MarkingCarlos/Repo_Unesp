...
void main_menu_functions(int menu_function) {
    ...
    switch (menu_function) {
        case CREATE: {
            char* input = get_input_from_window("Digite o nome do arquivo:", whatever,
                                                40);
...
int begin_terminal() {
    ...
    menu_t* main_menu = create_menu(ARRAY_SIZE(choices), choices, "Menu");
...
_Noreturn void* refresh_process_log() {
    ...
    while (1) {
        ...
        for (; i != NULL; i = i->next) {
            ...
            wattrset(process_window->text_window, COLOR_PAIR(1) | A_BOLD);
            wprintw(process_window->text_window, "Arquivo: ");
...
win_t* init_process_log() {
    ...
    const char title[] = "Status processo";
...
FWIN_CREATE(memory) {
    const char title[] = "Status memoria";
    ...
    while (1) {
        ...
        for (; i != NULL; i = i->next) {
            ...
            if (io_info->type == IO_LOG_PRINT) {
                ...
                wprintw(io_window->text_window, "Processo ");
                ...
                if ((io_log_fs->opt & IO_LOG_FS_READ) || (io_log_fs->opt & IO_LOG_FS_WRITE)) {
                    wprintw(io_window->text_window, "%s ",
                            ((io_log_fs->opt & IO_LOG_FS_READ) ? "lendo" :
                             (io_log_fs->opt & IO_LOG_FS_WRITE) ? "escrevendo" : "não definido"));
                } else {
                    wprintw(io_window->text_window, "%s ",
                            ((io_log_fs->opt & IO_LOG_FS_F_OPEN) ? "aberto" :
                             (io_log_fs->opt & IO_LOG_FS_F_CLOSE) ? "fechado" : "não definido"));
                }

                wattron(io_window->text_window, COLOR_PAIR(1) | A_BOLD);
                wprintw(io_window->text_window, "nao aclopado");
                ...
                wattron(io_window->text_window, COLOR_PAIR(1) | A_BOLD);
                wprintw(io_window->text_window, "foi pedido ");

                wattron(io_window->text_window, COLOR_PAIR(3) | A_NORMAL);
                wprintw(io_window->text_window, "%s", io_log_disk_req->read ? "ler" : "escrever");

                wattron(io_window->text_window, COLOR_PAIR(1) | A_BOLD);
                wprintw(io_window->text_window, " ooperação.\n");
            }
...
FWIN_REFRESH_TITLE(memory) {
    ...
    mvwprintw(win_mem->win.title_window, 1, 1, "restante: %d Kbytes", kernel->seg_table.remaining >> 10);
...
FWIN_REFRESH(memory) {
    ...
    LOOP {
        ...
        FOREACH(kernel->seg_table.seg_list, segment_t *) {
            ...
            wattrset(win_mem->win.text_window, COLOR_PAIR(1) | A_BOLD);
            wprintw(win_mem->win.text_window, "Paginas: ");

            wattrset(win_mem->win.text_window, COLOR_PAIR(3) | COLOR_PAIR(1) | A_BOLD);
            const int p_inuse_index = page_inuse_index(it);
            if (p_inuse_index == -1)
                snprintf(buffer, 100, "%d disponivel.\n", it->page_qtd);
            else
                snprintf(buffer, 100, "Usando %d of %d.\n", p_inuse_index,
                         it->page_qtd);
            wprintw(win_mem->win.text_window, "%s", buffer);
        }
...
FWIN_REFRESH(disk) {
    ...
    LOOP {
        ...
        for (; i != NULL; i = i->next) {
            ...
            wprintw(win_disk->win.text_window, "Processo ");
...
FWIN_REFRESH_TITLE(res_acq) {
    ...
    wprintw(res_acq_window->title_window, "Contagem semaforos: %d bloqueado: %d",
...
FWIN_REFRESH(res_acq) {
    ...
    LOOP {
        ...
        for (; i != NULL; i = i->next) {
            ...
            wprintw(res_acq_window->text_window, "Processo ");
            ...
            if (!(log->acq && log->blocked)) {
                ...
                wattron(res_acq_window->text_window, COLOR_PAIR(3) | A_NORMAL);
                wprintw(res_acq_window->text_window, "Bloqueado", log);

                wattron(res_acq_window->text_window, COLOR_PAIR(1) | A_BOLD);
                wprintw(res_acq_window->text_window, ", esperando por ");
            }

            wattron(res_acq_window->text_window, COLOR_PAIR(1) | A_BOLD);
            wprintw(res_acq_window->text_window, "semaforo ");
